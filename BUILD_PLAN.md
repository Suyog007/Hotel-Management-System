...
### 6.4 Guest messaging — ~~in-app chat~~ → WhatsApp

**Removed 2026-07-29.** The in-app guest ↔ reception chat (client component, guest bubble, staff inbox, server actions) was deleted; guests now message the hotel through the floating WhatsApp button (`components/public/floating-whatsapp.tsx`, mounted by `SiteFooter` on every public page). Staff reply from WhatsApp, not the dashboard.

- [x] Floating WhatsApp button on every public page, `wa.me` deep link with a pre-filled greeting
- [ ] ~~`/chat` guest page, `/dashboard/chat` staff inbox~~ — removed
- [x] `supabase/migrations/0016_drop_chat.sql` applied by the owner in the Supabase dashboard on 2026-07-29. Dropped the `conversations` / `messages` tables, the `on_new_message()` function, the `conversation_status` enum, and the `chat_new_message` notification template (2 conversations / 16 messages deleted). Verified against production: both tables return `PGRST205`, `notification_templates` is down to the three booking keys, and `notifications` / `bookings` / `rooms` / `profiles` / `site_settings` / `service_requests` are untouched

### 6.5 Sidebars

- [x] `components/staff/sidebar.tsx` — Menu, Services, Service requests
...
- [ ] Tap the floating WhatsApp button on any public page — it opens `wa.me/9779742529499` with the greeting pre-filled (WhatsApp app on mobile, web client on desktop)
...
- [x] `app/(staff)/dashboard/page.tsx` — 5 `Metric` tiles (arrivals / departures / occupancy / pending refunds / service requests) + today's-arrivals preview list
...
- 2026-07-29 — **Removed the in-app chat system; WhatsApp is now the only guest↔hotel messaging channel.** Deleted `components/chat/` (`realtime-chat.tsx` client component + `floating-chat-bubble.tsx`), the staff inbox `app/(staff)/dashboard/chat/` (list page, `[conversationId]` thread, `actions.ts`), `app/booking/[id]/chat-actions.ts`, and `test/chat-actions.test.ts` (16 tests, also stripped from `test-inventory.csv`). Call sites cleaned: `/booking/[id]` and `/my-bookings` no longer query `conversations`/`messages` or render the bubble (the `chat_error` search param and its error banner went too); the staff sidebar lost its **Chat** item and the dashboard lost the **Open chats** metric tile (5 tiles now) plus its `conversations` count query. Homepage FAQ blurb now points at WhatsApp instead of "Reception is online — start a chat". Guests reach the hotel via the existing floating WhatsApp button (`components/public/floating-whatsapp.tsx`, mounted by `SiteFooter` on every public page including `/booking/[id]` and `/my-bookings`) — `wa.me/9779742529499` with a pre-filled greeting, opened in a new tab. Staff/admin back-office pages deliberately have **no** WhatsApp button (staff are the hotel; they answer from WhatsApp itself). `docs/project-guide.html` updated (§tech-stack, §roles, §5.3, §6.1, §6.5). **DB teardown applied 2026-07-29:** `supabase/migrations/0016_drop_chat.sql` drops `messages` + `conversations` (cascade also clears their FK, index, triggers, RLS policies from 0002 and grants from 0012, and removes them from the `supabase_realtime` publication — `notifications` stays published), then `on_new_message()`, the `conversation_status` enum, and the `chat_new_message` seed row from 0003. `set_updated_at()` is shared and stays. `types/database.ts` regenerated by hand to drop the two table types + the enum. Migration was run by the owner in the dashboard (service-role key does data only); it is destructive — 2 conversations / 16 messages were deleted, and reading their bodies was deliberately not attempted. Order was safe either way here since the code no longer queries the tables. Numbered 0016 rather than 0015 because `0015_room_type_original_price.sql` landed on main first. Gates: type-check clean, production build succeeds with no `/dashboard/chat` route emitted, 175 tests pass (was 191 — the 16 removed are the deleted chat tests), lint warnings unchanged at 12. Post-apply smoke test against the live chatless DB: `/`, `/rooms`, `/my-bookings`, `/menu`, `/login`, `/gallery` all 200 with zero runtime errors, WhatsApp button present once per page, `/chat` 404.
...
- 2026-07-29 — **Photo upload replaces "paste a URL" everywhere images are set.** Only the gallery could actually take a file; every other image field was a URL box, which meant uploading to Storage by hand (or via the gallery) and copying the link back. Two new fields in `components/ui/`: **`image-upload-field.tsx`** (single photo — thumbnail preview of the current image, a file picker that previews the chosen file instantly via `URL.createObjectURL`, an × to clear, and the old URL box demoted to an "Or use a link instead" disclosure so off-site links still work) and **`image-list-field.tsx`** (multi-photo, for room types — existing photos as removable thumbnails, newly picked ones flagged `new #n` with a dashed border, and the newline-per-URL textarea kept behind a disclosure as the only way to reorder or paste). The list field keeps the textarea's *raw text* as its source of truth; deriving it from a parsed array swallowed the newline the moment you pressed Enter. Server side, `lib/storage.ts` gained `uploadFormImage` / `uploadFormImages` on top of the existing `uploadPublicImage`, both keyed on the fact that **an untouched `<input type="file">` still submits a zero-byte File** — so "was a photo actually picked?" is a size check, and a null return means "keep the URL the record already had". Wired into: menu items (`menu/`), room types (`rooms/`, multi, appended to the existing list so adding two photos doesn't wipe the set), testimonial avatars (`testimonials/`), hero/CTA section backgrounds (`sections/`), and gallery rows, which can now **replace** their file — the superseded Storage object is deleted best-effort, and the row id survives so page sections referencing that image don't break. Rejected files (wrong type, >10 MB, Storage error) redirect with the message instead of saving a half-updated row. **Bug caught while wiring:** clearing a photo produced `image_url: undefined`, which PostgREST drops from the PATCH — the × looked like it worked but the removal never persisted; menu and testimonials now write an explicit `null`. No migration and no schema change — the columns already existed. 11 new tests (`test/image-upload.test.ts`, `test/menu-photo-action.test.ts`) cover empty-picker/keep-URL, upload-and-replace, clear-to-null, oversized, wrong-type and Storage-failure paths. type-check + lint clean, 187 tests pass, production build succeeds. Fields were driven with Playwright against a throwaway `/ui-preview` route (pick, preview, remove, textarea sync) — route deleted, not committed; the real pages need admin auth so they were not clicked through logged in.
- 2026-07-29 — **Rooms page: physical rooms nested inside their room type, killing the duplicate second list.** Owner reported the page was still confusing. Root cause was structural, not cosmetic: it rendered two parallel top-level lists — "Room types" and "Rooms" — so a single-suite hotel saw "Premium Suite" twice, under two headings, with two search boxes and two create panels, and the type↔room relationship was only implied by a grouping header. Restructured to one list of room types; opening a type reveals its edit form *and* a "Rooms of this type" panel holding its physical rooms, each a `<details>` row (number, floor, status badge) expanding to a compact edit form, with "Add a room to <type>" at the bottom. The add form carries the type as a hidden `type_id` — the type is implied by where the form lives, so the type dropdown (and the chance of picking the wrong one) is gone from the create path; the edit form keeps a Type select so a room can still be reassigned. Server actions were **not** touched — `createRoom` already read `type_id` from the FormData, so this is a pure UI change. Care taken that each room `<form>` is a *sibling* of the type's form, never nested: nested `<form>` elements are invalid HTML and the inner one silently doesn't submit. Two things added to cover what the deleted second list used to provide: a status-summary strip at the top ("5 rooms in total · 2 available · 1 occupied · 1 maintenance · 1 cleaning") for cross-type visibility, and a "Rooms with no type" section that surfaces orphaned rooms left behind when a type is deleted (previously they'd have vanished from the page entirely under the new nesting). The room-type form also got shorter: name / price / max guests / web address and photos stay visible, while description, amenities, was-price and sort order fold into a "Show description, amenities & offer price" disclosure. Jargon softened throughout — "Base price" → "Price per night", "Slug" → "Web address", "Original price" → "Was-price", "Active" → "Bookable on the website". Room numbers now show in the collapsed type row ("Sleeps 5 · 3 rooms: 601, 602, 603") so the common question is answered without expanding anything. type-check + lint clean, 187 tests pass, production build succeeds. Layout driven with Playwright against a throwaway `/ui-preview` route seeded with 2 types × 5 rooms (the shape the old page handled worst) — route deleted, not committed; the real page needs admin auth so it was not clicked through logged in.
- 2026-07-29 — **Fix: "Expected string, received null" when adding a room (regression from the nested-rooms restructure).** Owner hit it on the live site trying to add a room to Premium Suite. Cause: nesting the rooms under their type meant rewriting the create form, and the Notes textarea was dropped in the process — but `parseRoom` still reads `notes: formData.get("notes")`, and `FormData.get()` returns **null** for a field the form doesn't render. zod's `.optional()` accepts `undefined` but not `null`, so every single room creation failed with that message; the field wasn't even one the owner was filling in. Fixed at both levels. Root cause: `optionalText` in all five validation modules (`rooms`, `cms`, `menu`, `services`, `staff`) now wraps its schema in `z.preprocess((v) => (v === null ? undefined : v), …)`, so an absent field and a blank one are treated identically — the same trap was one dropped input away in every other admin form. Surface: the Notes field is back on the nested add-room form, where it belonged. 5 regression tests added (`test/room-create-action.test.ts`) covering createRoom with no `notes` entry at all, createRoom with notes present, and the null-tolerance of `optionalText` in the rooms / menu / cms schemas; verified they **fail** against the pre-fix code (2 failed) and pass after, so the test is real rather than decorative. type-check clean, 192 tests pass, production build succeeds. **Also noted for the owner:** `rooms.room_number` is `unique` across the whole hotel (0001_initial_schema.sql), so the 602 they were typing into Premium Suite would have collided with Deluxe Twin's 602 even once the null bug was gone — the next free number is 604.
- 2026-08-17 — **Fix: /dashboard/rooms crashed with "Something went wrong" for every signed-in manager (regression from the amenity-picker PR #13).** Production hid the real error behind the RSC digest; reproduced locally with a minimal route: `TypeError: COMMON_ROOM_AMENITIES.map is not a function`. Cause: the rooms page (a server component) imported the `COMMON_ROOM_AMENITIES` constant from `components/ui/amenity-picker.tsx`, which is a `"use client"` module — every export of a client module reaches a server component as an opaque client-reference proxy, so calling `.map()` on it throws at request time. The crash sat *after* the auth/role checks, which is why the rest of the dashboard worked, anonymous visitors got redirected normally, and the build passed — only a signed-in manager opening the rooms page ever hit it. Ruled out data causes first: all `room_types.amenities` arrays and `rooms.status` values in prod are clean, and the July cleanup scripts touch nothing this page reads. Fix: moved the constant to `lib/amenities.ts` (no directive, importable from both worlds); `amenity-picker.tsx` keeps only the component. Verified via a temporary server-component route exercising the exact `amenityOptions` computation (renders 200), type-check clean, vitest unchanged vs main (the 9 `walkin-action` failures pre-exist on a clean checkout of main), production build passes. Lesson: shared constants must never live in `"use client"` files — the failure is invisible until runtime and only on the code path that touches them.
- 2026-08-17 — **Room-card price made the loudest thing on the card (owner request).** The `/rooms` price pill set the discounted price in caption-size text, so the offer barely read. The pill now stacks: struck original small on top, current price in display type + gold (`text-gold-light` on the onyx pill, matching the site's onyx/gold system); the with-dates "total" variant got the same treatment. Verified both variants in the running app across all six room cards; type-check clean. (PR #16) Follow-up per owner ("color seems off"): gold *text* on the onyx pill read mustard against the warm room photos — flipped the pill to solid gold with onyx text, the exact pairing of the "Book a room" button, so the price now echoes the booking CTA. Struck original and "/ night" in onyx at reduced opacity. (PR #18)
- 2026-08-18 — **Full physical-room inventory loaded from the owner's handwritten sheet (27 rooms, 8 types).** The DB previously held 9 placeholder rooms numbered 601-609, one per type. Applied via `scripts/sync-room-inventory.mjs` (dry-run by default, `--execute` to apply; idempotent — re-running produces an empty plan). Final layout: Deluxe Twin 301/302/303/305/403/404/405, Premium Double 304, Premium Suite 401, Deluxe Double 306/402/406, Standard Twin Family 601/701/801/901, Standard Twin (AC) 703/803, Standard Double Non-AC 603/704/804/904, Standard Twin Non-AC 602/702/802/902/903. `floor` set from the first digit (the hotel has no floor 5 or 2xx/1xx rooms). Rooms 601/602/603 were retyped in place rather than recreated — 601 carries the one real checked-in booking (94D57BE3), which survives untouched (price was snapshotted at booking time). 604-609 deleted; 24 rooms created. **Extra bed** recorded as an `'Extra bed available'` amenity on Deluxe Twin and Deluxe Double only (owner confirmed the scribbled Deluxe Twin mark means yes, Premium Suite's blank means no) — amenity, not a schema column, so no owner-run migration was needed. **Deluxe Single room type deleted** (owner's choice — it has no physical rooms on the sheet). Prices/names/sort untouched. Owner's sheet also says "font-size on Room page" — explicitly deferred, not done. Verified post-apply: 27 rooms, 8 types, 1 booking intact.
- 2026-08-18 — **Room map: bus-seat-style live occupancy view at `/dashboard/room-map` (client request).** Two building cards side by side — Building A (Premium Suite / Premium Double / Deluxe Twin / Deluxe Double, floors 3-4) and Building B (the four Standard types, floors 6-9); assignment is by room-type slug, not floor, so a re-shuffle follows the type. Floors render top-down like a real elevation with a "Ground" baseline; each room is a colour-coded tile: oxblood solid = occupied, gold solid = arriving today (pending/confirmed stay covering today, not yet checked in), dashed silver = cleaning, onyx = maintenance, pale emerald = free — legend on top, per-building "n / N occupied" in the header. Occupied is derived from `bookings.status='checked_in'` with **no date filter** (an overdue checkout still holds its room — the current live booking is exactly that case) OR `rooms.status='occupied'`; tiles with a live stay link to `/booking/[id]` and carry a title-tooltip + sr-only text with guest name and dates. Presentational pieces live in `components/staff/room-map.tsx` (server-safe, no client JS — tooltips are `title` attrs); the page queries three ways in parallel (rooms+type join, checked-in, due-today) and joins in code. Sidebar: new "Room map" item (Building2 icon) in the Today group, receptionist+; the Overview "Occupancy" metric tile now links here. Verified with Playwright screenshots at 1440px and 390px against a throwaway seeded `/ui-preview` route (deleted before commit) — all five tile states legible, floors wrap cleanly on mobile; type-check clean, production build emits the route.
- 2026-08-18 — **Room map folded into the dashboard Overview + rich hover cards (owner feedback: standalone page left blank space).** The `/dashboard/room-map` route and its sidebar item are gone; `RoomMapSection` (a self-fetching server component in `components/staff/room-map.tsx`) now renders between the metric tiles and "Arriving today" on `/dashboard`, with the legend inline next to the section heading and the two buildings on an `xl:grid-cols-2` grid. The plain `title` tooltip became a pure-CSS hover card (`group-hover` + `group-focus-within`, no client JS): room number + status dot, type name, "Sleeps N · Rs X / night" (query now joins `max_guests`/`base_price`; stays fetch `booking_code`/`guests_count`), and for a live stay the guest name, code, party size, dates with in-house/due-today, and a "Click to open booking" hint — free rooms say "No booking today". `BuildingCard` swapped `overflow-hidden` for `rounded-t-[4px]` on the header band so the card can escape upward; verified via the throwaway seeded `/ui-preview` route (deleted) that a top-floor tile's card clears the building header without clipping. The Overview occupancy tile lost its now-dead link. type-check clean, production build no longer emits `/dashboard/room-map`.
- 2026-08-18 — **Overdue checkouts surfaced everywhere + in-app notification bell (owner-approved plan after booking 94D57BE3 sat un-checked-out past its 2026-08-17 departure).** Deliberately NO auto-checkout — only the desk knows if the guest actually left; the system makes the miss loud instead. (1) Bookings page: departures query went `eq(check_out, today)` → `lte`, section renamed "Departures due", overdue rows get a danger "Overdue · N days" badge; `paid_amount` added to the select. (2) Check out button now confirms when money is outstanding ("Rs X still outstanding … Check out anyway?") via a new optional `confirmMessage` prop on `SubmitButton`. (3) Overview: danger banner above the metric tiles listing each overdue stay (#room, guest, due date) with a "Review departures" link. (4) Room map: new `overdue` tile state — oxblood with a gold ring + alarm icon, own legend entry, hover card says "past check-out, not checked out"; counted in the building "n / N occupied". (5) Bell: `notifications` table (schema-day leftover, previously unused) now feeds `components/staff/notification-bell.tsx` (client dropdown, unread badge, mark-all-read server action using the self-only RLS policies) mounted in the BackOfficeNav header — desktop-only since the mobile drawer's close button owns that corner; the Overview banner covers mobile. (6) Daily cron `/api/cron/overdue-checkouts` (vercel.json 04:15 UTC = 10:00 NPT, CRON_SECRET-gated like the reviews cron) deletes unread `overdue_checkout` notifications (stale either way) and inserts one fresh summary per active staff profile; copy renders from the `overdue_checkout` notification template — seeded via `scripts/seed-overdue-notification-template.mjs` (idempotent, run 2026-08-18), editable/toggleable at /admin/templates, with hardcoded fallback copy if the row vanishes. Verified: Playwright screenshots of the overdue tile + open bell; cron triggered locally against the live DB (CRON_SECRET is empty in `.env` so a temp env var was used) → `{"overdue":1,"notified":1}` and a correctly rendered notification row for the retained super_admin. **Assumes CRON_SECRET is set in Vercel env** (the reviews cron already depends on it). type-check clean, production build emits the new route.
- 2026-08-24 — **Google site name fix: WebSite JSON-LD + "Resturant" typo (owner report: search results show "hotelvardani.com" above the link instead of the hotel name).** Google's site-name feature reads `WebSite` structured data from the homepage; the site only emitted `Hotel`/`FAQPage`/`HotelRoom` schemas + `og:site_name`, so Google fell back to the bare domain. `HotelJsonLd` now also emits a `WebSite` schema (`name` = `site_settings.hotel_name`, `alternateName` = ["Hotel Vardani", "hotelvardani.com"], `url` = site URL) — same settings fetch, second script tag. Separately fixed the DB value `site_settings.hotel_name` "Hotel Vardani and Resturant" → "Hotel Vardani and Restaurant" (one-off service-role script, owner picked the spelled-correct long form as canonical); the typo was leaking into the search-result title, `og:site_name`, and the Hotel schema. Google picks the change up on its next homepage recrawl (days–weeks; homepage reindex can be requested in Search Console). type-check clean.
- 2026-08-24 — **Group-stay guidance on /rooms (owner report: a 10-guest search dead-ended).** A party larger than the largest room (currently sleeps 6) tripped `exceedsCapacity` on every type, so the page filtered everything out and showed the "Sold out for these dates / try different dates" empty state — factually wrong (nothing sold out; no date ever helps). Now `groupStay` (party > largest capacity, computed from all active types, not the price-filtered set) skips the capacity filter so available rooms stay bookable, and an accent banner above the grid explains: largest sleeps N, a party of G needs ≥ ceil(G/N) rooms, book the first below and repeat — or call, with a `tel:` link built from the first number in `site_settings.contact_phone` (the field holds two numbers separated by "·"; naive digit-stripping would have mashed them into one). The genuine all-booked case keeps an accurate copy variant ("All rooms are booked for these dates"). Per-room capacity is still enforced downstream: the detail-page form clamps to `max_guests` and `initiateBooking`/walk-in re-check server-side, so the banner can't create over-capacity bookings. Verified against the live DB via local dev render: guests=10 shows the banner ("sleeps 6 … at least 2 rooms", tel:+9779742529499) with all 8 room cards; guests=4 unchanged (no banner, small rooms filtered). type-check clean.
- 2026-08-24 — **Guest stepper cap 10 → 20 (owner follow-up to the group-stay work).** The hero-search guest stepper was hardcoded to stop at 10, so a party of 11+ couldn't even express its size. Now capped by a `MAX_GUESTS = 20` constant matching the `guests_count` ceilings in `lib/validation/rooms.ts` and `lib/validation/staff.ts`; any party above the largest room's capacity lands on the /rooms group-stay banner, so big numbers are a valid search, not an error. type-check clean.
- 2026-08-24 — **Group booking cart: a big party books all its rooms from the website in one go (owner picked pick-your-own cart over a one-click suggested combo).** When the party exceeds the largest room, /rooms swaps the card grid for `components/public/group-room-picker.tsx` (client): per-type +/- steppers (capped by that type's availability and a global `MAX_ROOMS_PER_BOOKING = 6`), a sticky bar tracking "N rooms · sleeps X of G · Rs total", and once capacity covers the party a checkout form (one name/email/phone + per-type price summary) posts to the new `initiateGroupBooking` (`app/rooms/group-actions.ts`). Under the hood the booking intent cookie became multi-room: `bookingIntentSchema` now carries `rooms[]` (single-room flow stores a 1-element array — one shape, one finalize path; in-flight intents from before the deploy just re-parse as expired, 15-min TTL makes that moot). `findAvailableRooms(type, n)` generalizes `findAvailableRoom`. Guests split across rooms server-side: 1 per room minimum, then fill to capacity in selection order. `verifyAndCreateBooking` loops the inserts **with compensating rollback** — if any insert hits the `bookings_no_overlap` exclusion (23P01 race), already-created rows are deleted so the guest never holds half a reservation; per-booking audit rows + per-booking confirmation emails (each carries its own access_token link), then group redirects to `/my-bookings?booked=N` (new success banner; single room keeps the `/booking/[id]` redirect). No schema change — the group is N ordinary bookings sharing guest + dates, so staff flows (check-in, cancel, refund) work per room untouched. Verified: type-check clean; `booking-verify-action` tests updated to the intent shape + new 2-room group test (30 pass; walkin failures pre-exist); dev render against live DB shows the cart at guests=10 and the classic grid at guests=4.
- 2026-09-03 — **Admin alert system overhaul (all 7 review recommendations).** The bell was a once-a-day, desktop-only banner fed by a single cron; now it's a live, multi-event pipeline. (1) *Producers:* new `lib/notify-staff.ts` fans one in-app notification out to every active staff member; wired into online booking (`verifyAndCreateBooking` — one notification per group, not per room), walk-in creation, and cancellation; the overdue cron now reuses the same helper. `lib/notification-types.ts` is the single registry of consumed keys (staff_new_booking, staff_cancellation, overdue_checkout) with hardcoded default copy so notifications work even before the template rows exist. (2) *Live bell:* `components/staff/notification-listener.tsx` subscribes to Supabase realtime `postgres_changes` on the signed-in member's `notifications` rows (published since 0007, previously unused) — each insert pops a sonner toast (new dep, Toaster mounted in ResponsiveShell) and `router.refresh()`es the server-rendered bell. Mounted once per back-office layout via `notification-live.tsx`. (3) *Mobile:* the bell now also renders in the mobile top bar (`ResponsiveShell` grew a `topbarAction` slot; bell dropdown got an `align="right"` mode) — previously a phone user in /admin had no notification surface at all. (4) *Templates:* migration **0017** (⚠️ run by hand in the Supabase dashboard) deletes the dead guest-facing notification templates (booking_confirmed/booking_cancelled — no guest bell exists), seeds overdue_checkout (previously only via a hand-run script) + staff_cancellation, and refreshes staff_new_booking copy; `/admin/templates` now renders from the code registry (with per-key "when this fires" descriptions and a seeded-or-not callout) instead of listing raw table rows nothing consumes. (5) *Error hygiene:* new `lib/friendly-error.ts` — raw Supabase `error.message` no longer goes into `?error=` URLs (9 action files swept; Zod + curated upload messages stay verbatim); bespoke saved/error divs on /dashboard/bookings and /dashboard/walk-in replaced with the shared `StatusNote`. (6) *Silent failures surfaced:* `writeAudit` now logs a dropped insert; `sendTemplatedEmail` failures/missing templates write an `email_failed` audit row visible at /admin/audit; `markAllNotificationsRead` logs its errors; new staff-voiced error boundaries `app/(admin)/admin/error.tsx` + `app/(staff)/dashboard/error.tsx` (shared `back-office-error.tsx`, shows the digest) replace the guest-branded fallback. Sentry left as the existing TODO — needs a DSN/account decision. (7) *Confirm dialogs:* `window.confirm` in DeleteButton/SubmitButton replaced by an in-app `components/ui/confirm-dialog.tsx` (themed, Escape/overlay cancel, `requestSubmit(submitter)` preserves formAction/formNoValidate semantics); zero `window.confirm` left in the codebase. Verified: type-check clean, lint clean (pre-existing warnings only), prod build clean, new `test/notify-staff.test.ts` 6/6 pass, affected suites green (walkin-action's 9 failures confirmed identical on the unmodified file — pre-existing).
- 2026-09-03 — **Super admin now lands on the /dashboard Overview after login** (PR #27). login/verify-otp previously redirected super_admin to /admin → /admin/settings — a config page, not where the day starts. All back-office roles now land on the shared Overview; the bare /admin index also redirects there (admin sections stay one click away in the unified sidebar). Role gating unchanged (`test/middleware.test.ts` 9/9).
- 2026-09-04 — **Fixed broken back-office link (dead "Service requests" tile).** Live-URL audit of every admin/staff page: all 13 admin pages + dashboard pages render with no console errors, and every internal `/admin` and `/dashboard` href maps to a real route — except the dashboard Overview's "Service requests" metric tile, which linked to `/dashboard/service-requests` (404 — the service-requests feature was removed but the tile was orphaned, showing a hardcoded "—" value). Removed the dead tile and squared the metrics grid to 4 columns (`md:grid-cols-4`); dropped the now-unused `ConciergeBell` import. `lib/validation/services.ts` remains but is imported nowhere (harmless dead file, left as-is). tsc + build clean. No DB change.
- 2026-09-04 — **Room actions from the room map.** Occupied tiles already opened the booking detail, but rooms with no guest today (free / being cleaned / maintenance) weren't clickable — so a room stuck in cleaning had no "mark ready" from the map. Those tiles now open a small click popover (`components/staff/room-tile-menu.tsx`) with the status changes that make sense: cleaning → mark ready, maintenance → mark available, free → send to cleaning / set maintenance (the last with a confirm). New staff-level `setRoomStatus` action (in bookings/actions.ts, next to markRoomReady): whitelists available/cleaning/maintenance, refuses to touch an occupied room ("manage it from the guest's booking"), audited, returns to the overview. `returnBase` regex widened to accept the bare `/dashboard` path so the map's `redirect_to=/dashboard` sticks. New `test/room-status-action.test.ts` (5: mark ready + redirect, free→maintenance, invalid status rejected, occupied refused, off-site redirect ignored). tsc + lint + build clean; 13 green across room-status + checkout suites. No DB change.
- 2026-09-04 — **Walk-in form: pick a specific room number.** The walk-in form only let staff choose a room *type*; the action auto-assigned whichever room was free. Added a second "Room number" dropdown that populates with the rooms of the chosen type that are free for the selected dates, defaulting to "Any available room (N free)" (auto-assign, unchanged behaviour). New client component `components/staff/walk-in-room-picker.tsx` owns the dates + type + room-number fields and fetches availability from a new staff-gated route `app/api/dashboard/available-rooms` (session role check, then `listAvailableRooms()` — new helper in `lib/availability.ts` returning `{id, room_number}[]`, excludes maintenance rooms, filters by date overlap). `walkInBookingSchema` gained an optional `room_id`; the action honours it only when the room belongs to the chosen type, isn't under maintenance, and is still free — otherwise it falls back to auto-assign (never hard-fails on a room taken between form-load and submit). Reads only; no DB change. tsc + lint + build clean; validation suite green; walk-in suite's 9 failures pre-exist (fake-supabase limitation, unchanged — new room_id path isn't exercised by those tests).
- 2026-09-04 — **Back-office booking detail page with inline front-desk actions.** Clicking an occupied room on the dashboard room map (or a booking code / arrival card anywhere in the back office) used to open the guest-facing `/booking/[id]` receipt — no staff actions. New `app/(staff)/dashboard/bookings/[id]/page.tsx` is a staff view of the booking (status/payment badges, stay, guest, receipt with outstanding balance) with every front-desk action inline: **check in**, **check out** (with the payment-collection dialog), **extend stay** (manager+), **mark room ready** (when the room is in cleaning), and **cancel** (pending/confirmed, with reason). Room-map tiles, the overview "arriving today" cards, and the bookings-list codes now link here instead of the guest page. To keep staff on the page after acting, the four bookings actions (`checkIn/checkOut/extendStay/markRoomReady`) and `cancelBooking` gained an optional `redirect_to` form field — validated to `^/dashboard/[A-Za-z0-9/_-]*$` (off-site values ignored), defaulting to the old bookings-list redirect so existing call sites are unchanged. `bail` → `bailTo(base, msg)` so errors return to the same page too. Guest `/booking/[id]` untouched (my-bookings, email links, token access). Tests: checkout suite +2 (redirect_to honoured, off-site ignored) = 8; cancellation/staff/verify/notify/middleware all green (53 total across touched suites); type-check + lint + build clean. No DB change; live booking data only read, never mutated by this work.
- 2026-09-03 — **Collect payment at checkout.** Pay-at-hotel bookings used to stay "Unpaid" forever — checkout never touched payment, and nothing else could record one after creation. Now the bookings-page "Check out" on an unpaid booking opens a payment dialog (`components/staff/checkout-button.tsx`): method (cash/Khalti/eSewa) + optional reference, primary button submits with `collect=1` — the server tops up `paid_amount`, flips `payment_status` to paid, inserts a `payments` row (same shape as walk-in) and audits the collection; "Check out unpaid" remains as the escape hatch for walk-away/comp cases, and a settled booking keeps the one-click button. Success banner reads "Checked out — collected Rs X." New `test/checkout-action.test.ts` (6 tests: full/partial collection, no-collect, already-paid no-duplicate, bad provider, wrong status). type-check + lint + build clean.
- 2026-09-03 — **Migration 0017 applied to the live DB** (data-only, via service-role script — no dashboard needed): dead notification templates booking_confirmed/booking_cancelled deleted, overdue_checkout + staff_cancellation seeded, staff_new_booking copy refreshed. notification_templates now holds exactly the three consumed keys, all active; /admin/templates is fully editable.
- 2026-09-05 — **Back office declutter, second pass: `/admin/templates` goes list-first, and the last mismatched controls are normalized.** The 2026-07-29 pass converted nine CRUD pages to `ManageList` + `CreatePanel` but skipped four, and `/admin/templates` was the one still rendering the original pattern in full: every email template *and* every notification type as a permanently-open form — each with a 6-row HTML-body textarea plus a 3-row plain-text one — stacked down a single page, so reaching the last notification template meant scrolling past every email body. Now one scannable row per template (key as the title, subject/description as the subtitle) grouped under **Email** / **In-app notification**, with search over key + subject + body + variables, an "Off" badge for `is_active = false`, and a "Not seeded" badge replacing the inline amber warning for the migration-0017 gap. `{{variable}}` hints moved off the card header into a chip row inside the open form, next to the fields that actually take them. Verified with a throwaway `/ui-preview` route driven by Playwright (deleted before commit): **0 forms in the DOM while collapsed, 1 after expanding a row, still 1 after expanding a second** — the one-open-at-a-time invariant holds, and the page now opens in one screen instead of eight stacked forms.

  The button audit that prompted this found `size="sm"` already applied consistently (the 07-29 rule held), so the remaining offenders were the controls *around* the buttons. **Nine hand-rolled `<select>` elements** — in `/admin/staff` (×2), `/dashboard/walk-in` (×4), `checkout-button.tsx`, `walk-in-room-picker.tsx` (×2) — carried inline classes that had drifted from `Input`: `bg-background` instead of `bg-card`, no shadow, no focus ring, no hover border, and a `h-9` role dropdown sitting next to `h-10` inputs on the same staff row. All now use the shared `Select`; measured in the browser, `Input` and `Select` both render at exactly 40px. **`Pager` was copy-pasted byte-for-byte into five pages** (audit, gallery, testimonials, cancellations, reviews), each drawing its prev/next as bare `<a class="px-3 py-1.5">` — a third button height next to the `sm` (36px) and default (44px) buttons. Extracted to `components/ui/pager.tsx` on `buttonVariants({ variant: "outline", size: "sm" })`; the links now measure 36px, matching every other `sm` button, and it self-hides at `pages <= 1` so callers can drop their guards. Loose page-level submits in templates/settings/staff/branding/walk-in/reviews wrapped in `FormActions` so every form ends with the same bordered footer strip. Net −111 lines. type-check + lint clean (no new warnings), production build passes; the 9 `walkin-action.test.ts` failures are pre-existing and untouched (`walk-in/actions.ts` is not in this diff).

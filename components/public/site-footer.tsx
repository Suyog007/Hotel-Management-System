...
export async function SiteFooter() {
  ...
  return (
    <>
      <footer className="mt-24 bg-ink text-white-wash/65">
        <div className="container flex flex-col gap-4 py-12 md:flex-row md:items-center md:justify-between md:py-10">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <span aria-hidden className="inline-block h-2.5 w-2.5 rounded-full bg-rosewood-light" />
            <span className="font-display text-base font-bold text-white-wash">
              {s.hotel_name ?? "Hotel"}
            </span>
            {s.address && (
              <a
                ...
                className="inline-flex items-center gap-1 text-sm text-white-wash/65 transition-colors hover:text-rosewood-light"
              >
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-white-wash/65">
            {PHONES.map((p) => (
              <a
                key={p.tel}
                href={`tel:${p.tel}`}
                aria-label={`${p.label}: ${p.display}`}
                className="inline-flex items-center gap-1 transition-colors hover:text-rosewood-light"
              >
            ...
            {s.contact_email && (
              <a
                ...
                className="transition-colors hover:text-rosewood-light"
              >
                {s.contact_email}
              </a>
            )}
            <div className="flex items-center gap-2">
              {SOCIAL_LINKS.map(({ name, href, icon: Icon }) => (
                <a
                  ...
                  className="grid h-8 w-8 place-items-center rounded-full border border-white-wash/20 text-white-wash/65 transition-colors hover:border-rosewood-light/60 hover:text-rosewood-light"
                >
                  <Icon className="h-3.5 w-3.5" />
                </a>
              ))}
            </div>
            <span className="font-label md:ml-2 text-[11px] text-white-wash/45">
              © {new Date().getFullYear()} {s.hotel_name ?? "Hotel"}
            </span>
            <Link
              href="/login"
              className="font-label text-[11px] text-white-wash/35 transition-colors hover:text-white-wash/70"
            >
              Staff
            </Link>
          </div>
